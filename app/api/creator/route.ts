...
export async function GET(req: NextRequest) {
  ...
  if (ig) {
    ...
    const [profile, reels, demog, daily] = await Promise.all([
    ...
    if (profile?.status?.code === 200) {
      const { profile: p, stats: s } = profile
    ...
    if (reels?.status?.code === 200 && reels?.posts?.length) {
      const posts = reels.posts
    ...
    if (demog?.status?.code === 200 && demog?.demographic) {
      ...
      result.igTopCountries = d.followersCountries?.slice(0, 5).map((c: any) => ({
        name: c.name, pct: Math.round(c.value * 1000) / 10
      })) || []
      result.igTopCities = d.followersCities?.slice(0, 3).map((c: any) => ({
        name: c.name, pct: Math.round(c.value * 1000) / 10
      })) || []
      ...
      result.igAgeDistribution = d.ages?.map((a: any) => ({
        age: a.name, pct: Math.round(a.percent * 1000) / 10
      })) || []
    ...
    if (daily?.meta?.code === 200 && daily?.dailyStats?.length) {
      ...
      const totalDelta = stats.reduce((s: number, d: any) => s + (d.deltaFollowers || 0), 0)
      result.igFollowerWachstum7d = totalDelta
      result.igAvgDailyLikes = avg(stats.map((d: any) => d.deltaLikes || 0))
      result.igAvgDailyComments = avg(stats.map((d: any) => d.deltaComments || 0))
      result.igQualityScore = daily.summaryStats?.qualityScore || 0
  ...
  if (tt) {
    ...
    const [profile, videos, daily] = await Promise.all([
    ...
    if (profile?.status?.code === 200) {
      ...
      result.ttGender = p.gender || ''
      result.ttAge = p.age || ''
      result.ttCountry = p.country || ''
      if (!result.fullName) result.fullName = p.name || ''
    }

    if (videos?.status?.code === 200 && videos?.posts?.length) {
      const posts = videos.posts
      ...
      const topPost = posts.reduce((best: any, p: any) => {
        const v = p.postDetails?.videoViews || 0
        return v > (best?.postDetails?.videoViews || 0) ? p : best
      }, null)
      if (topPost) {
        result.ttTopVideoViews = topPost.postDetails?.videoViews || 0
        result.ttTopVideoUrl = topPost.postDetails?.postUrl || ''
        result.ttTopVideoLikes = topPost.postDetails?.likes || 0
      }
    }
    ...
    if (daily?.meta?.code === 200 && daily?.dailyStats?.length) {
      const stats = daily.dailyStats
      const totalDelta = stats.reduce((s: number, d: any) => s + (d.deltaFollowers || 0), 0)
      result.ttFollowerWachstum7d = totalDelta
      result.ttAvgDailyViews = avg(stats.map((d: any) => d.deltaViews || 0))
      result.ttAvgDailyLikes = avg(stats.map((d: any) => d.deltaLikes || 0))
