...
const tierStyle: Record<string, string> = {
  'Nano': 'text-gray-400 bg-gray-800/80 border border-gray-700/50',
  ...
  return (
    <div className="flex min-h-screen bg-[#0A0A0A]">
      ...
      <main className="flex-1 md:ml-60 min-h-screen">
        <div className="border-b border-white/[0.06] px-8 py-4 bg-[#0A0A0A]/80 backdrop-blur sticky top-0 z-20">
          <h1 className="text-white font-semibold text-lg">Discovery</h1>
          <p className="text-gray-500 text-xs mt-0.5">Influencer finden und zur Liste hinzufügen</p>
        </div>

        <div className="p-8 space-y-6 max-w-5xl">
          {/* Such-Bereich */}
          <div className="bg-[#141414] rounded-2xl border border-white/[0.06] p-6">
            <h2 className="text-white font-medium text-sm mb-1">Influencer suchen</h2>
            <p className="text-gray-600 text-xs mb-5">Gib einen Instagram- oder TikTok-Handle ein</p>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-4">
              <div>
                <label className="text-gray-400 text-xs block mb-1.5 flex items-center gap-1.5">
                ...
                <input type="text" value={igHandle} onChange={e => setIgHandle(e.target.value)}
                  ...
                  className="w-full bg-[#0A0A0A] border border-white/[0.08] rounded-lg px-3 py-2.5 text-white text-sm focus:outline-none focus:border-[#7F77DD]/50 transition-colors"/>
              </div>
              <div>
                <label className="text-gray-400 text-xs block mb-1.5 flex items-center gap-1.5">
            ...
            <button onClick={search} disabled={loading || (!igHandle && !ttHandle)}
              className="w-full md:w-auto px-6 py-2.5 rounded-xl bg-[#7F77DD] text-white text-sm font-medium hover:bg-[#534AB7] transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2">
              {loading ? (
                <>
            ...
            <div className="bg-[#141414] rounded-2xl border border-white/[0.06] p-6">
              <div className="flex items-start justify-between mb-6">
                <div className="flex items-center gap-4">
                  <div className="w-16 h-16 rounded-full bg-gradient-to-br from-[#7F77DD] to-purple-700 flex items-center justify-center text-white text-xl font-bold">
                    {(result.name || igHandle || ttHandle || '?').slice(0, 1).toUpperCase()}
                  </div>
                  <div>
                    <h2 className="text-white font-semibold text-lg">{result.name || igHandle || ttHandle}</h2>
                ...
                <button onClick={addToCreators} disabled={adding || added}
                  className={`px-4 py-2 rounded-xl text-sm font-medium transition-all flex items-center gap-2 ${added ? 'bg-emerald-600 text-white' : 'bg-[#7F77DD] text-white hover:bg-[#534AB7]'} ${adding ? 'opacity-50' : ''}`}>
                  {adding ? (
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {[
                  ...
                  <div key={s.label} className="bg-[#0A0A0A] rounded-xl p-3 border border-white/[0.06]">
                    <div className="text-gray-600 text-[10px] uppercase tracking-wide font-medium">{s.label}</div>
                    <div className="text-white text-sm font-semibold mt-1">{s.value}</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          {history.length > 0 && (
            <div className="bg-[#141414] rounded-2xl border border-white/[0.06] p-6">
              <h2 className="text-white font-medium text-sm mb-1">Zuletzt gesucht</h2>
              <p className="text-gray-600 text-xs mb-4">{history.length} Suchen</p>
              <div className="space-y-2">
                {history.map((h, i) => (
                  <div key={i} className="flex items-center justify-between p-3 rounded-xl bg-[#0A0A0A] border border-white/[0.04]">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-full bg-[#7F77DD]/20 flex items-center justify-center text-[#7F77DD] text-xs font-semibold">
                        {(h.name || '?').slice(0, 1).toUpperCase()}
                      </div>
                      <div>
                        <div className="text-white text-xs font-medium">{h.name || 'Unbekannt'}</div>
          ...
          {!result && history.length === 0 && !loading && (
            <div className="bg-[#141414] rounded-2xl border border-white/[0.06] p-12 text-center">
              ...
              <h3 className="text-white text-sm font-medium mb-1">Influencer entdecken</h3>
              <p className="text-gray-600 text-xs max-w-sm mx-auto">Gib einen Handle ein und lade echte Daten direkt zu deiner Liste — Follower, Engagement, Tier, Werte und mehr.</p>
            </div>
          )}
        </div>
      </main>
